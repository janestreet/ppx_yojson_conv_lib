...
(** [yojson_of_ref conv r] converts the value [r] of type ['a ref] to an Yojson. Uses
    [conv] to convert values of type ['a] to an Yojson. *)
val yojson_of_ref : ('a -> Yojson.Safe.t) -> 'a ref -> Yojson.Safe.t

(** [yojson_of_lazy_t conv l] converts the value [l] of type ['a lazy_t] to an Yojson.
    Uses [conv] to convert values of type ['a] to an Yojson. *)
val yojson_of_lazy_t : ('a -> Yojson.Safe.t) -> 'a lazy_t -> Yojson.Safe.t

(** [yojson_of_option conv opt] converts the value [opt] of type ['a option] to an Yojson.
    Uses [conv] to convert values of type ['a] to an Yojson. *)
val yojson_of_option : ('a -> Yojson.Safe.t) -> 'a option -> Yojson.Safe.t

(** [yojson_of_pair conv1 conv2 pair] converts a pair to an Yojson. It uses its first
...
(** [yojson_of_triple conv1 conv2 conv3 triple] converts a triple to an Yojson using
...
(** [yojson_of_list conv lst] converts the value [lst] of type ['a list] to an Yojson.
    Uses [conv] to convert values of type ['a] to an Yojson. *)
val yojson_of_list : ('a -> Yojson.Safe.t) -> 'a list -> Yojson.Safe.t

(** [yojson_of_array conv ar] converts the value [ar] of type ['a array] to an Yojson.
    Uses [conv] to convert values of type ['a] to an Yojson. *)
val yojson_of_array : ('a -> Yojson.Safe.t) -> 'a array -> Yojson.Safe.t

(** [yojson_of_hashtbl conv_key conv_value htbl] converts the value [htbl] of type
    [('a, 'b) Hashtbl.t] to an Yojson. Uses [conv_key] to convert the hashtable keys of
    type ['a], and [conv_value] to convert hashtable values of type ['b] to Yojsons. *)
...
(** [yojson_of_opaque x] converts the value [x] of opaque type to an Yojson. This means
    the user need not provide converters, but the result cannot be interpreted. *)
val yojson_of_opaque : 'a -> Yojson.Safe.t

(** [yojson_of_fun f] converts the value [f] of function type to a dummy Yojson. Functions
    cannot be serialized as Yojsons, but at least a placeholder can be generated for
    pretty-printing. *)
val yojson_of_fun : ('a -> 'b) -> Yojson.Safe.t

(** {6 Conversion of Yojsons to OCaml-values} *)

(** [Of_yojson_error (exn, yojson)] the exception raised when an Yojson could not be
...
(** [of_yojson_error reason yojson]
    @raise Of_yojson_error (Failure reason, yojson). *)
val of_yojson_error : string -> Yojson.Safe.t -> 'a

(** [of_yojson_error_exn exc yojson]
    @raise Of_yojson_error (exc, yojson). *)
...
(** [ref_of_yojson conv yojson] converts Yojson [yojson] to a value of type ['a ref] using
    conversion function [conv], which converts an Yojson to a value of type ['a]. *)
val ref_of_yojson : (Yojson.Safe.t -> 'a) -> Yojson.Safe.t -> 'a ref

(** [lazy_t_of_yojson conv yojson] converts Yojson [yojson] to a value of type ['a lazy_t]
    using conversion function [conv], which converts an Yojson to a value of type ['a]. *)
val lazy_t_of_yojson : (Yojson.Safe.t -> 'a) -> Yojson.Safe.t -> 'a lazy_t

(** [option_of_yojson conv yojson] converts Yojson [yojson] to a value of type ['a option]
    using conversion function [conv], which converts an Yojson to a value of type ['a]. *)
val option_of_yojson : (Yojson.Safe.t -> 'a) -> Yojson.Safe.t -> 'a option

(** [pair_of_yojson conv1 conv2 yojson] converts Yojson [yojson] to a pair of type
    ['a * 'b] using conversion functions [conv1] and [conv2], which convert Yojsons to
...
(** [triple_of_yojson conv1 conv2 conv3 yojson] converts Yojson [yojson] to a triple of
    type ['a * 'b * 'c] using conversion functions [conv1], [conv2], and [conv3], which
    convert Yojsons to values of type ['a], ['b], and ['c] respectively. *)
...
(** [list_of_yojson conv yojson] converts Yojson [yojson] to a value of type ['a list]
    using conversion function [conv], which converts an Yojson to a value of type ['a]. *)
val list_of_yojson : (Yojson.Safe.t -> 'a) -> Yojson.Safe.t -> 'a list

(** [array_of_yojson conv yojson] converts Yojson [yojson] to a value of type ['a array]
    using conversion function [conv], which converts an Yojson to a value of type ['a]. *)
val array_of_yojson : (Yojson.Safe.t -> 'a) -> Yojson.Safe.t -> 'a array

(** [hashtbl_of_yojson conv_key conv_value yojson] converts Yojson [yojson] to a value of
    type [('a, 'b) Hashtbl.t] using conversion function [conv_key], which converts an
    Yojson to hashtable key of type ['a], and function [conv_value], which converts an
...
(** [opaque_of_yojson yojson]
    @raise Of_yojson_error when attempting to convert an Yojson to an opaque value. *)
val opaque_of_yojson : Yojson.Safe.t -> 'a

(** [fun_of_yojson yojson]
    @raise Of_yojson_error when attempting to convert an Yojson to a function. *)
